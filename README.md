...
### 문제 정의
한국사능력검정시험(한능검)은 공무원·공기업 가산점 등에 활용되어 매년 수십만 명이 응시한다.
하지만 학습 도구로 일반 LLM을 쓰면 두 가지 문제가 있다.

1. **환각(할루시네이션)** — 한국사는 연도·인물·사건 순서가 정확해야 하는데, LLM은 근거 없이
   그럴듯하게 지어내곤 한다. 근거 없는 오답을 정답처럼 학습하면 오히려 해롭다.
2. **개인화 부재** — 일반 챗봇은 매번 새로 시작해 "내가 어느 시대·주제에 약한지"를 기억하지 못한다.

### 차별점
- **기출 근거 제시형** — 출제·채점은 **직접 구축한 기출 문제 DB에서만** 이루어지고, 개념 답변도
  근거(개념·사료 DB / 위키백과)를 붙인다. 근거를 못 찾으면 지어내지 않고 "확인된 자료 없음"으로
  답한다. (환각 방지를 **아키텍처로 강제**)
- **오답노트 기반 취약점 학습** — 사용자가 틀린 문제의 시대·주제를 State(`wrong_tags`)에 누적한다.
  세션이 쌓일수록 "당신은 일제강점기 정치사가 약하다"처럼 진단하고 그 영역을 집중 출제한다.
- **실제 기출 통계 반영** — 국사편찬위원회 공식 최근 22개 회차(57~78회) 기출 1,100문항을 직접
  분석해 시대·주제 빈출도를 집계했고, 약점 진단 시 "이 영역은 최근 22회 중 약 N회 출제된 빈출
  영역"이라는 **검증된 근거 수치**를 제시한다.
