...
검색해 선택**한다. 정확한 문항·정답·해설이 보장된다.

**출제 대상 우선순위**(`select_quiz`):
① 명시적으로 요청한 시대·주제("조선 후기 경제 문제 줘") → ② 취약 영역 집중 요청 + 오답노트
존재("취약 영역 문제 줘") → ③ 그 외 일반 요청("문제 내줘")은 랜덤. 즉 오답노트가 쌓여 있어도
일반 요청에는 랜덤 출제하고, 사용자가 취약 영역을 명시할 때만 오답노트 기반으로 집중 출제한다.
